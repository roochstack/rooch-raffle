...
function SlotCounter(
  ...
  const startAnimation = useCallback(() => {
    ...
    setTimeout(() => {
      Promise.all(
        Array.from(slotCounterRef.current!.querySelectorAll<HTMLSpanElement>('span.block')).map(
  ...
  return (
    <span
      key={key}
      ref={slotCounterRef}
      className={mergeClassNames(containerClassName, 'inline-block whitespace-nowrap')}
    >
          ...
          return (
            <span
              ...
              className={mergeClassNames(
                'inline-block text-center align-middle',
                separatorClassName
              )}
            >
              {v}
