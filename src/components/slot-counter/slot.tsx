...
function Slot(
  ...
  // const [fontHeight, setFontHeight] = useState(30);
  const fontHeight = 30;
  const setFontHeight = noop;
  ...
  const renderDummyList = () => {
    return dummyListState.map((dummyNumber, slotIndex) => (
      <span
        key={slotIndex}
        className={mergeClassNames('block', numberClassName)}
        aria-hidden="true"
      >
        {dummyNumber}
      </span>
  ...
  return (
    <span
      className={mergeClassNames(
        'inline-block transform overflow-hidden text-center align-middle',
        charClassName
      )}
      style={{
    ...
    >
      <span
        ref={numbersRef}
        className={mergeClassNames('block', numberSlotClassName)}
        style={{
          transition: 'none',
      ...
      >
        {didMount ? (
          <>
            ...
            {renderDummyList()}
            <span
              className={mergeClassNames('block', numberClassName, valueClassName)}
              ref={itemRef}
            >
              {bottomValue}
            </span>
          ...
          <span className={mergeClassNames('block', numberClassName)} aria-hidden="true">
